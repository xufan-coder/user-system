...
docker run --name ${DOCKER_CONTAIN} --restart=always --privileged=true --net=host -m ${MAX_MEM} -p ${HOST_PROT}:${APP_PORT} -v ${HOST_LOG_DIR}:${APP_LOG_DIR} -v ${HOST_DIR}:${APP_DIR} -v /home/docker/static:/home/docker/static -d ${DOCKER_IMG}
