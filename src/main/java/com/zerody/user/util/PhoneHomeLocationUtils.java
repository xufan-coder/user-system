 ...
 */
public class PhoneHomeLocationUtils {
  ...
  private static  void initData() {
    if (dataByteArray == null) {
      ...
      int readBytesLength;
      try (InputStream inputStream = PhoneHomeLocationUtils.class.getClassLoader().getResourceAsStream(PHONE_DAT_FILE_PATH)) {
  ...
  public static PhoneNumberInfo lookup(String phoneNumber) {
    ...
    int left = 0;
    int right = Math.abs(phoneRecordCount);
    while (left <= right) {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(PhoneHomeLocationUtils.lookup("15914349502"));
