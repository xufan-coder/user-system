...
public class GenerateAccount {
    ...
    public static String generateIDCard() {
        ...
        // 随机生成地区码（前6位）
        sb.append(generateRandomRegionCode());

        // 随机生成生日信息（中间8位）
        sb.append(generateRandomBirthday());

        // 随机生成顺序码（后3位）
        sb.append(generateRandomSequenceCode());
    ...
    // 随机生成地区码（前6位）
    static String generateRandomRegionCode() {
        // 以模拟值 "440300"（深圳市）为例
        return "440300";
    }

    // 随机生成生日信息（中间8位）
    static String generateRandomBirthday() {
        Random random = new Random();
        int year = random.nextInt(30) + 1980;  // 假设年龄在 20 到 50 岁之间
        int month = random.nextInt(12) + 1;
        int day = random.nextInt(28) + 1;  // 假设月份为 2 时不考虑闰年的情况
        return String.format("%04d%02d%02d", year, month, day);
    }

    // 随机生成顺序码（后3位）
    static String generateRandomSequenceCode() {
        Random random = new Random();
        int sequence = random.nextInt(899) + 100;  // 顺序码从 100 到 999
        return String.format("%03d", sequence);
    }

    // 计算校验码（最后1位）
    static char calculateCheckCode(String idCardNumber) {
